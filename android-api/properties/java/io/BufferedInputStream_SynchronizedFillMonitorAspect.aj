...
public aspect BufferedInputStream_SynchronizedFillMonitorAspect implements rvmonitorrt.RVMObject {
	...
	pointcut BufferedInputStream_SynchronizedFill_fill() : (call(* BufferedInputStream.fill(..)) && target(i) && !cflow(call(synchronized * *.*(..)))) && MOP_CommonPointCut();
	before () : BufferedInputStream_SynchronizedFill_fill() {
		BufferedInputStream_SynchronizedFillRuntimeMonitor.fillEvent();
	}
