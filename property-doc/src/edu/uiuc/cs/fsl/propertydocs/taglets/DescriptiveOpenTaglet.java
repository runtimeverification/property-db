...
/**
* This Taglet handles to opening of Descriptive text comments
*
* @author Patrick Meredith
*
*/

public class DescriptiveOpenTaglet implements Taglet {
    private static final String NAME = "descriptive.open";
  	private static final String dir = Standard.htmlDoclet.configuration().destDirName;

    private File stats = new File(dir + File.separator + "__properties" + File.separator + "descriptive.stats");

    private Set<PositionWrapper> seenDocs = new HashSet<PositionWrapper>();
    private int chars = 0; //number of descriptive chars
    private int words = 0; //number of descriptive words
    private int lines = 0; //number of descriptive lines
    ...
    public static void register(Map tagletMap) {
       DescriptiveOpenTaglet tag = new DescriptiveOpenTaglet();
     ...
     * and collect statistics on <b>all</b> the descriptive sections in the 
     * document.
     *
     * Statistics will only be collected if we have not already seen an 
     * descriptive.open tag for this document.  We keep track of the document
    ...
    private void handleTags(Tag[] tags){
      boolean inDescriptive = false; 
      int c = 0; //character count
      int w = 0; //word count
      int l = 0; //line count
      for(Tag tag : tags){
        if(tag.name().equals("@descriptive.open")){ 
          inDescriptive = true;
        }
        else if(tag.name().equals("@descriptive.close")){
          inDescriptive = false;
        }
        else if(tag.name().equals("Text") && inDescriptive){
          ...
          || tag.name().equals("@inheritDoc")) && inDescriptive){
           System.err.println("ERROR: " + tag.name() + " inside descriptive environment in comment near " 
      ...
      if(inDescriptive) {
         System.err.println("ERROR: @descriptive.open tag in comment near " 
